...
//   const { user } = useContext(Usercontext);
//   const [efforts, setEfforts] = useState([]);
//   const [rejectedEffortUniqueKey, setRejectedEffortUniqueKey] = useState(null);
//   const [rejectedEffortId, setRejectedEffortId] = useState(null);
//   const [remarks, setRemarks] = useState('');
...
const Approvals = () => {
  const { user } = useContext(Usercontext);
  const [efforts, setEfforts] = useState([]);
  // Change rejectedEffortId to store a unique identifier for the specific effort
  const [rejectedEffortUniqueKey, setRejectedEffortUniqueKey] = useState(null);
  const [remarks, setRemarks] = useState('');
  ...
  useEffect(() => {
    fetch(`http://52.88.246.113:8085/pendingApprovals/${user}`)
      ...
      .catch(err => {
        console.error('Failed to fetch efforts:', err);
        console.log('Error fetching pending efforts.');
      });
  }, [user]);

  const handleAction = async (effort, action) => {
    // Construct the unique key for the current effort
    const currentEffortUniqueKey = effort.info.cohortCode + effort.info.id + effort.info.date;

    if (action === 'reject') {
      // Validate if a reason is selected from the dropdown
      if (!remarks.trim()) {
        console.log('Please select a reason for rejection.');
        return;
      }
      // Check if "Other" is selected but no custom remark is provided in the text area
      if (remarks === 'Other' && !otherRemark.trim()) {
        console.log('Please specify your reason for rejection when "Other" is selected.');
        return;
      }
    }
    ...
    effort.status = action;
    if (action === 'reject') {
      effort.remarks = remarks === 'Other' ? otherRemark : remarks;
    }

    // Optimistically remove the effort from the list
    setEfforts(efforts.filter(eff => {
      // Filter using the same unique key logic
      const effUniqueKey = eff.info.cohortCode + eff.info.id + eff.info.date;
      return effUniqueKey !== currentEffortUniqueKey;
    }));

    // Reset rejection states
    setRejectedEffortUniqueKey(null); // Reset the unique key
    setRemarks('');
    setOtherRemark('');
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to update');
      console.log(`Effort ${action}ed successfully`);
    } catch (err) {
      console.error(err);
      console.log(`Failed to ${action} effort`);
    }
  };
      ...
      <div className="row justify-content-center">
        <div className="col-12 col-lg-10">
          ...
          {efforts.length === 0 ? (
            ...
            efforts.map(effort => {
              // Define the unique key for each effort item
              const effortUniqueKey = effort.info.cohortCode + effort.info.id + effort.info.date;

              return (
                <div key={effortUniqueKey} className="card shadow-sm border-0 mb-4">
                  <div className="card-body">
                    <div className="d-flex justify-content-between align-items-center mb-2">
                      <h6 className="mb-0 text-dark">
                        {effort.info.iD} <span>({effort.info.cohortCode})</span>
                    ...
                    <div className="d-flex gap-3">
                      ...
                      <button
                        className="btn btn-sm btn-outline-danger"
                        onClick={() => {
                          setRejectedEffortUniqueKey(effortUniqueKey); // Use the unique key here
                          setRemarks('');
                          setOtherRemark('');
                        }}
                      >
                    ...
                    {rejectedEffortUniqueKey === effortUniqueKey && ( // Compare with the unique key
                      <div className="mt-3 form-group">
                        <label htmlFor={`remarks-select-${effortUniqueKey}`} className="form-label visually-hidden">Rejection Reason</label>
                        <select
                          id={`remarks-select-${effortUniqueKey}`}
                          className="form-select mb-2"
                          value={remarks}
                          onChange={(e) => {
                            setRemarks(e.target.value);
                        ...
                        {remarks === 'Other' && (
                          <textarea
                            id={`other-remarks-textarea-${effortUniqueKey}`}
                            className="form-control mt-2"
                            placeholder="Please specify your reason..."
                            value={otherRemark}
                        ...
                        <div className="mt-3">
                          ...
                          <button
                            className="btn btn-sm btn-secondary"
                            onClick={() => {
                              setRejectedEffortUniqueKey(null); // Clear the unique key
                              setRemarks('');
                              setOtherRemark('');
                            }}
                          >
