...
const Notification = () => {
  ...
  useEffect(() => {
    fetch(`http://52.88.246.113:8085/approvalStatus/${user}`)
      .then(res => res.json())
      .then(data => setEfforts(data))
      .catch(err => {
        console.error('Failed to fetch efforts:', err)
        // Using console.log instead of alert for error messages
        console.log('Error fetching pending efforts.')
      })
  }, [user]) // Added user to dependency array as it's used in fetch
                  ...
                  {sortedEfforts.map(effort => (
                    <tr key={effort.info.id}>
                      ...
                      <td className={`fw-bold ${
                        ...
                        {formatStatus(effort.status)} {/* Changed here */}
                      </td>
                      <td>{effort.topic}</td>
