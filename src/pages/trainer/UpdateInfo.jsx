...
const UpdateInfo = () => {
    ...
    if (user) {
      const fetchTrainerData = async () => {
        try {
          ...
          const response = await fetch(`http://52.43.62.125:8085/trainer/${user}`); // Assuming an endpoint like /trainer/{tmId}
          if (response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://52.43.62.125:8085/updateInfo", {
        method: 'POST',
        headers: {
