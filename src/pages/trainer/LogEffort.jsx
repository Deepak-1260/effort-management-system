...
const LogEffort = () => {
  ...
  useEffect(() => {
    const fetchCohorts = async () => {
      if (form.tmId) {
        try {
          const res = await fetch(`http://52.88.246.113:8081/getAllocatedCohorts/${form.tmId}`);
          if (res.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://52.88.246.113:8085/effortupdate', {
        method: 'POST',
        headers: {
      ...
      <div className="card-body px-5 py-5">
        ...
        >
          ...
          {[
            ...
            { name: 'date', label: 'Date', icon: 'calendar-date-fill', type: 'date', max: getTodayDate() }
          ].map(({ name, label, icon, type = 'text', readOnly = false, required = true ,max}) => (
            <div className="col-md-6" key={name}>
              ...
              <div className="input-group has-validation">
                ...
                {required && name !== 'tmId' && (
                  <>
                    {name === 'hours' && (Number(form.hours) < 0 || form.hours === '') ? (
                      <div className="invalid-feedback">Please enter valid hours (non-negative).</div>
                    ) : (
                      <div className="invalid-feedback">{label} is required.</div>
                    )}
                  </>
...
//       });
//       const data = await res.text();
//       alert(data); // Consider replacing alert with a custom modal for better UX
//       setForm({
//         tmId: user || '',
//         cohortCode: '',
//         mode: 'Offline', // Changed back to 'Offline' as per initial state
//         reason: '',
//         hours: '',
//         date: '',
//         topic: '',
...
//     } catch (err) {
//       console.error('Submission error:', err);
//       alert('Failed to log effort.'); // Consider replacing alert with a custom modal for better UX
//     }
//   };
...
//                     required={required}
//                     max={max}
//                     min={name === 'hours' ? "0" : undefined} // Ensures hours are not negative
//                     style={{
//                       fontSize: '0.9rem',
//                       backgroundColor: name === 'tmId' ? '#e9ecef' : undefined,
//                       color: name === 'tmId' ? '#6c757d' : undefined,
...
//                   />
//                 )}
//                 {/* Corrected placement and syntax for invalid-feedback */}
//                 {required && name !== 'tmId' && (
//                   <>
//                     {name === 'hours' && (Number(form.hours) < 0 || form.hours === '') ? (
//                       <div className="invalid-feedback">Please enter valid hours (non-negative).</div>
